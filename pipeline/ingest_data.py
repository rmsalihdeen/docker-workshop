...
@click.option('--month', type=int, default=1, help='Month for the taxi data (1-12)')
...
def run(year, month, pg_user, pg_pass, pg_host, pg_port, pg_db, table_name, chunksize):
    ...
    for df_chunk in tqdm(df_iter):
        if first:
            df_chunk.head(0).to_sql(name=table_name, 
                                    con=engine, 
                                    if_exists='replace')
